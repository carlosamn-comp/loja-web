...
erDiagram
    ...
    USUARIO {
        bigint id PK
        varchar tipo "discriminador ADMIN/LOJA/CLIENTE"
        varchar role
        varchar nome
        varchar email UK
        varchar senha
...
|---------------|--------|----------|----------------------------|
| `id` | `id` | `bigint` (PK, identity) | `@Id`, `@GeneratedValue(IDENTITY)` |
| (discriminador) | `tipo` | `varchar` (`ADMIN`/`LOJA`/`CLIENTE`) | `@DiscriminatorColumn(name="tipo")` |
| `nome` | `nome` | `varchar(120)` not null | `@Column(nullable=false, length=120)`, `@NotBlank` |
...
> nunca a expõe. O **administrador** também é uma subclasse de `Usuario`
> (`@DiscriminatorValue("ADMIN")`, role `"ADMIN"`), persistido na tabela `usuario`.

### CarrinhoItem → tabela `carrinho_item`

Carrinho **persistente** do cliente (sobrevive a fechar/reabrir a página).

| Atributo Java | Coluna | Tipo SQL | Anotações JPA |
|---------------|--------|----------|---------------|
| `id` | `id` | `bigint` (PK, identity) | `@Id`, `@GeneratedValue(IDENTITY)` |
| `cliente` | `cliente_id` | `bigint` (FK → `usuario.id`) not null | `@ManyToOne(fetch=LAZY)`, `@JsonIgnore` |
| `produto` | `produto_id` | `bigint` (FK → `produto.id`) not null | `@ManyToOne(fetch=EAGER)` |
| `quantidade` | `quantidade` | `integer` not null | `@Column(nullable=false)` |
...
create table usuario (
    tipo varchar(31) not null,                 -- discriminador: ADMIN, LOJA ou CLIENTE
...
alter table carrinho_item   add constraint fk_carrinho_cliente       foreign key (cliente_id)   references usuario;
alter table carrinho_item   add constraint fk_carrinho_produto       foreign key (produto_id)   references produto;
```
